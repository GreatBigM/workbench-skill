...
- **归档漏 spec 复审**：归档检查清单三项（scheme 吸收 design / spec 复审更新 / 产出落知识库）全绿才算完成——漏 spec 复审即归档未完成
- **check 越界定标准**：标准由 goal 定、方法由 check 给、结果落 tasks——check 不得出现 goal 之外的门槛（如 goal ≥94 则 check 不得写 ≥90）
- **基线双头**：基线数字以 tasks「测试数据」为单一权威源，goal 标准表/check 验收的基线列引用此处
